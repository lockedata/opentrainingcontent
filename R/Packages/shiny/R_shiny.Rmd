...
## Quick examples
[The cost of coffee](//stef.shinyapps.io/cost_of_coffee)
```{r Basic_R_shiny-1, eval=FALSE, echo = TRUE}
IEDA::runIEDA()
```
...
A paragraph
```{r Basic_R_shiny-8, eval=FALSE}
shinyApp(
  ui = fluidPage(textAreaInput(id="charbox", rows=3, 
...
### Interactive tables
```{r Basic_R_shiny-13, eval=FALSE}
shinyApp(
  ui = fluidPage(DT::dataTableOutput("datatable")  ),
  server = function(input, output) {
    output$datatable <- DT::renderDataTable(head(iris,5))
  }
)
```

### Charts
```{r Basic_R_shiny-14, eval=FALSE}
shinyApp(
