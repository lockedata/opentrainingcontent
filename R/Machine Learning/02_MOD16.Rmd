...
```{r rnorm, fig.cap="Continuous outcome variable"}
library(ggplot2)
set.seed(777)
...
```{r xy, fig.cap="Trying to apply a line of best fit"}
x_b<-y_b+rnorm(1000)
...
```{r 02_MOD16-2, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
...
```{r 02_MOD16-4, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
...
```{r 02_MOD16-7}
library(caret)
confusionMatrix(ifelse(fitted(model,type="response")>.5,
                       "Control","Impaired"), 
                diagnosis[-1])
```
